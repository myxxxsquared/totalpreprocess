...
{
    // totalpreprocess height width input output
    if(argc != 5)
        throw "useage: totalpreprocess height width input output";

    const int height = atoi(argv[1]);
    const int width = atoi(argv[2]);
    const char* const input_file = argv[3];
    const char* const output_file = argv[4];

    Preprocess process;
    process.process(height, width, input_file);
    // Mat image, tmpimage;
    // tmpimage = cv::imread("../testdata/img.jpg");
    // image.create(height, width, tmpimage.type());
